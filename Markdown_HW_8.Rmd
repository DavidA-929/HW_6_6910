...
By plotting the standardized residuals against the fitted values we can see that the spread of the standardized residuals, fluctuate as the fitted values increase. There also seems to be an outlier. The outlier may be of interest to inspect. Hence, we can say that the equal variance assumption is not satisfied. 
...
Futhermore, we can see that there doesn't exist a path from 1 to 5, which means that $\tau_1 - \tau_5$ is not estimable. However there is a path from 1 to 6, so $\tau_1 - \tau_6$ is estimable.
