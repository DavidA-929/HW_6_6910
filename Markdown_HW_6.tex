...
From the plot above we can see that the burner level 2 is consistently
above or below the 0 line, except for when the salt level is 4, which is
an indication that the the errors do not have mean zero and therefore
the two-way main effects model is probably not the best model to
represent the data.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#Y_i_var <- tapply(spaghetti.data$weight, spaghetti.data$trtmt, var);}
\CommentTok{#max(Y_i_var)/min(Y_i_var)}
\end{Highlighting}
\end{Shaded}
...
values we can see that the spread of the standardized residuals is
roughly centered around zero. However there seems to be one aparrent
outlier. Hence, with these two results we feel comfortable to say that
the equal variance assumption is roughly satisfied.

Assumption (c): The error are normally distributed:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#max(abs(spaghetti.data$z))}
\end{Highlighting}
\end{Shaded}
